...
% Papameters obtained from
% "Capturing the bursting dynamics of a two-cell inhibitory network 
%  using a one-dimensional map" - Victor Matveev, Amitabha Bose, Farzan Nadim
 


function dydt = burstDE(t, Y, Z)
...
%gL0=gL;
gL0=2.3; % The conductance of the leak channel seems playing an significant role here
...
(1-Y(7))*0.5*(1 + tanh(slope*(ylag1(1)-vthresh)))/tgamma - Y(7)*0.5*(1 + tanh(slope*(vthresh-ylag1(1))))/tauSyn;
(1-Y(8))*0.5*(1 + tanh(slope*(ylag2(2)-vthresh)))/tgamma - Y(8)*0.5*(1 + tanh(slope*(vthresh-ylag2(2))))/tauSyn;
